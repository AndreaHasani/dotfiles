...
def rofi_choose(options):
    ...
    stdoutdata, _ = rofi_process.communicate("\n".join(title).encode()) # Pipe our windows through rofi, rofi will always output a order number for the list
    return int(stdoutdata.decode('utf-8').replace("\n", '')) # We get the number output and make sure its a int 

def use_xprop(x): # class to get our window name and app used (to make it look nicier :D )
